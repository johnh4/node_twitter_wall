...
function init() {
	...
	//$('body').on('click', onDocumentMouseDown);
  ...
  birdEl.className = 'bird';
	...
	function twerk(bird){
		...
		window.setInterval(function(){
			...
			if(Math.random() >= .5){
				...
				//moveTo.x = Math.random() * 4000 - 2000;
				//moveTo.y = Math.random() * 4000 - 2000;
				//moveTo.z = Math.random() * 4000 - 2000;
				//moveTo.z = ;
	...
	var details = document.createElement('div');
	details.className = 'details';
	details.innerHTML = "KEYSTONE XL AD VISIBLE ON WHITE HOUSE WEBSITE";
	element.appendChild(details);
	...
	button.addEventListener( 'click', function ( event ) {
		...
		//prev = current - 1; 
		if(current.prevO){
			//current = current - 1 + spliceOffset;

			current = current.prevO
			//prev--;
			console.log('current', current);
			//camTarg = objA[current].position;
...
function render() {
	...
	if(resetCam){
		//console.log('reseting cam rotation');
...
function onDocumentMouseDown( event ) {
	...
	//console.log("this.threeObj", this.threeObj);
	var prevCamPos = camera.position;
  ...
  /*
	var vector = new THREE.Vector3( ( event.clientX / window.innerWidth ) * 2 - 1, - ( event.clientY / window.innerHeight ) * 2 + 1, 2000 );
	projector.unprojectVector( vector, camera );

	var raycaster = new THREE.Raycaster( camera.position, vector.sub( camera.position ).normalize() );
	console.log('raycaster', raycaster);
	console.log('raycaster.ray.origin.x', raycaster.ray.origin.x, 'camera.position.x', camera.position.x);
	console.log('raycaster.ray.origin.y', raycaster.ray.origin.y, 'camera.position.y', camera.position.y);
	console.log('raycaster.ray.origin.z', raycaster.ray.origin.z, 'camera.position.z', camera.position.z);

	console.log("objA", objA);
	console.log("targets.table", targets.table);
